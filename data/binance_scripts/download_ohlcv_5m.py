        ...
        try:
            df1 = pd.DataFrame(
                client.get_historical_klines(
                    ...
                    int(pd.Timestamp.now().timestamp() * 1000) if end_time is None
            ...
            df1 = df1.astype(
                {'o': float, 'h': float, 'l': float, 'c': float, 'v': float, 'q': float, 'bv': float, 'bq': float})
            df1.to_sql(name="bnc_kline_5m", con=mysql_conn, if_exists="append", index=False,
                       method=insert_on_conflict_update)
            logging.info(f'{i}, {symbol}, completed')
        except KeyboardInterrupt:
            break
        except (ConnectionError, ConnectionResetError, BinanceAPIException) as e:
            traceback_info = traceback.format_exc()
            logging.warning(f'{i}, {symbol}, error \n{traceback_info}')
            traceback.print_exc()
            continue
    logging.info(f'download_ohlcv_5m completed')


if __name__ == "__main__":
    main('2024-12-20 05:40:00')
