    ...
    for period in periods:
        for symbol in symbols:
            try:
                ...
                res = rq.get(
                    base_url + f'/futures/data/takerlongshortRatio?' + '&'.join([f'{k}={v}' for k, v in {
                ...
                if df.empty:
                    ...
                    time.sleep(4 / 3)
                    continue
                df.columns = ['lsr', 'l', 's', 'ts0']
                df['symbol'] = symbol
                df['lsr'] = df['lsr'].replace('Infinity', np.nan)
                df.to_sql(name="bnc_taker_ls_ratio_5m", con=mysql_conn, if_exists="append", index=False,
