...
async def task_generator(queue_task, loop):
    ...
    for date in pd.date_range('2024-01-01', '2024-01-31', freq='1D')[::-1]:
...
async def request_worker(queue_task, queue_content, stop_event):
    ...
    while not stop_event.is_set():
        ...
        while not stop_event.is_set():
            try:
                res = await session.get(f"https://data.binance.vision/data/futures/um/daily/aggTrades/{symbol}/{symbol}-aggTrades-{date.strftime('%Y-%m-%d')}.zip")
...
async def mysql_worker(queue_content, stop_event, loop):
    ...
    # upserter = ddb.TableUpserter(dbPath="dfs://market_data", tableName="bnc_aggtrades", ddbSession=session, keyColNames=["a", "symbol"])
    ...
    while not stop_event.is_set():
        ...
        with zipfile.ZipFile(io.BytesIO(content)) as zip_ref:
            with zip_ref.open(zip_ref.namelist()[0]) as file_aggtrades:
                df_aggtrades = pd.read_csv(file_aggtrades)

        df_aggtrades.columns = ['a', 'p', 'q', 'f', 'l', 'ts', 'm']
        df_aggtrades['symbol'] = symbol
        df_aggtrades['t'] = pd.to_datetime(df_aggtrades['ts'] * 1e6)
        df_aggtrades = df_aggtrades.astype({'p': float, 'q': float})
        df_aggtrades['v'] = df_aggtrades['p'] * df_aggtrades['q']
        df_aggtrades['n'] = df_aggtrades['l'] - df_aggtrades['f']
        df_aggtrades = df_aggtrades[['ts', 'symbol', 'a', 't', 'f', 'l', 'p', 'q', 'v', 'n', 'm']]

        # data = df_aggtrades.values
        ...
        # upserter.upsert(df_aggtrades)
        appender.append(df_aggtrades)
        print(f"{pd.Timestamp.now()}, mysql_worker, {symbol}, {date}, executed.")
        ts = int(pd.Timestamp.now().timestamp() * 1e3)
        params = (date, symbol, ts)
        insert_query = f"INSERT INTO bnc_aggtrades_from_website_record (date, symbol, ts) VALUES (%s, %s, %s)"
