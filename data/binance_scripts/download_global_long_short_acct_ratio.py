    ...
    for period in periods:
        for symbol in symbols:
            try:
                ...
                res = rq.get(
                    base_url + f'/futures/data/globalLongShortAccountRatio?' + '&'.join([f'{k}={v}' for k, v in {
                ...
                if df.empty:
                    ...
                    time.sleep(4 / 3)
                    continue
                df.columns = ['symbol', 'l', 'lsr', 's', 'ts0']
                df['lsr'] = df['lsr'].replace('Infinity', np.nan)
                df.to_sql(name="bnc_global_ls_acct_ratio_5m", con=mysql_conn, if_exists="append", index=False,
